...
public interface ChatParticipantRepository extends JpaRepository<ChatParticipant, Long> {
       ...
       Optional<ChatParticipant> findByUserIdAndChatRoomId(@Param("userId") Long userId,
       ...
       int updateLastActivityAt(@Param("userId") Long userId, @Param("chatRoomId") Long chatRoomId,
       ...
       @Query("UPDATE ChatParticipant p SET p.isActive = false, p.leftAt = :leftAt " +
                     "WHERE p.user.id = :userId AND p.chatRoom.id = :chatRoomId")
       int deactivateParticipant(@Param("userId") Long userId, @Param("chatRoomId") Long chatRoomId,
       ...
       @Query("UPDATE ChatParticipant p SET p.role = :role " +
                     "WHERE p.user.id = :userId AND p.chatRoom.id = :chatRoomId")
       int updateParticipantRole(@Param("userId") Long userId, @Param("chatRoomId") Long chatRoomId,
       ...
       List<ChatParticipant> findInactiveParticipants(@Param("chatRoomId") Long chatRoomId,
       ...
       @Query("SELECT p FROM ChatParticipant p " +
                     ...
                     "AND p.isActive = true " +
                     "ORDER BY p.joinedAt ASC")
       List<ChatParticipant> findByRoleInChatRoom(@Param("chatRoomId") Long chatRoomId,
                     @Param("role") ChatParticipant.ParticipantRole role);
       ...
       @Query("SELECT p FROM ChatParticipant p " +
                     ...
                     "AND p.user.id = :userId " +
                     "AND p.isActive = true")
       Optional<ChatParticipant> findByChatRoomIdAndUserIdAndIsActiveTrue(@Param("chatRoomId") Long chatRoomId,
                     @Param("userId") Long userId);
       ...
       @Query("SELECT COUNT(DISTINCT p.user.id) FROM ChatParticipant p " +
                     ...
                     "AND (c.owner.id = :sellerId OR c.coOwner.id = :sellerId) " +
